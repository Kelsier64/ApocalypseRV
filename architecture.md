...
RakerGrab 是怪物的暫態抓取狀態機，PlayerGrab 持有唯一抓取者、輸入、HUD 與鏡頭鎖。0.6 秒前搖後重新驗證雙臂可達性和遮擋，再開始固定 2 秒倒數；6–10 次需求只抽一次，80% 以整數比例判定。咬合第 0.38 秒獨立扣血一次，避開一般受傷冷卻，同一 tick 解除存活玩家的輸入、鏡頭和 HUD；怪物自行進入 0.35 秒 RELEASE，死亡回呼重入清理時不重啟流程。玩家模式 GRABBED 優先於 SEATED，save/load 沿用 NORMAL gate 拒絕，無存檔欄位變更。駕駛事件和 Chassis 輪詢輸入都封鎖，仍保留車輛物理與鬆油門回收。雙方 tree_exiting、轉場、受傷、死亡、失去接觸／支撐、強制離座共用解除入口。

RakerPoseModifier 使用 SkeletonModifier3D 在動畫後依實際臉向修正三節頸骨，僅更改旋轉，權重 35%／35%／30%；180°/s、yaw ±90°、站立上 30°／蹲姿上 40°，一般下壓 25°。站立 reach／hold／bite 只低頭看玩家、不再加脊椎俯身；此時下壓可至 65°，先求解頸部轉動造成的臉部位置偏移，復原輸入姿勢後再套用平滑限速。眼嘴之間的可見臉部中心作追視及鏡頭目標。低姿態／座位咬擊仍使用脊椎 CCD 前探，单節最多 55°，每次求解後再次校正臉向與限角。肩膀／臉位置以骨架局部座標快取，避免渲染與移動 RV 的物理幀不同步。抓咬動畫由獨立 per-actor AnimationLibrary 播放；雙臂解析 IK 與接觸檢查共用臂長與肘部方向。手掌座標使用模型正確掌面符號，前臂分攤 65% 軸向扭轉、手腕完成朝向；指根朝玩家越過肩膀，末節以當前掌面及近節指骨形成的平面向掌心屈曲 45°，拇指朝頸部，避免手指折回怪物自身。PlayerGrab 前拉鏡頭最多 28 cm，以半徑 9 cm 球體掃掠阻擋，肩膀接觸點始終取原鏡頭錨點，解除時復位。存活解除同時恢復當前玩家視角的滑鼠捕捉和地面輸入回呼；座位維持其原有輸入所有權，轉場／死亡不搶回鏡頭。傷害幀暗紅閃光獨立衰減。
