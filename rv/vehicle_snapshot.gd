...
static func capture(rv: Node3D) -> Dictionary:
	...
	return {"version": VERSION, "id": rv.persistent_id, "transform": rv.global_transform,
		"linear": rv.linear_velocity, "angular": rv.angular_velocity, "health": rv.current_chassis_health,
...
static func validate(data: Dictionary) -> bool:
	if data.get("version", 0) != VERSION or not data.has_all(["equipment", "wheels", "transform", "materials", "items", "fuel", "fuel_capacity", "material_capacity", "item_capacity", "id", "health", "engine", "gear", "handbrake", "linear", "angular"]):
		return false
	...
	if not data.transform is Transform3D or not data.linear is Vector3 or not data.angular is Vector3 or data.wheels.size() != 4:
		return false
	if not data.id is String or not data.engine is bool or not data.handbrake is bool or not data.gear is int or data.gear < -1 or data.gear > 4 or not _number(data.health): return false
	...
	return MaterialStorage.new().valid_amounts(data.materials)
...
static func apply(rv: Node3D, data: Dictionary) -> bool:
	...
	rv.current_chassis_health = clampf(data.health, 0.0, rv.max_chassis_health)
	rv.chassis_destroyed = rv.current_chassis_health <= 0.0
	rv.energy.engine_running = data.engine and not rv.chassis_destroyed and rv.current_fuel > 0.0
	rv.gear = data.gear
