...
scene=bpy.data.scenes['MONSTER_REFINED_V017']
bpy.context.window.scene=scene
rig=bpy.data.objects['Refined017_Rig']
mesh=bpy.data.objects['Refined017_Mesh']
...
for track in rig.animation_data.nla_tracks:
    ...
    for frame in range(int(action.frame_range[0]),int(action.frame_range[1])+1):
        ...
        # Lips and teeth roots intentionally embed into the skin/gums.
        # Audit unchanged body self-collisions and tooth-to-tooth collisions separately.
        mats=[ev.polygons[t.polygon_index].material_index for t in ev.loop_triangles]
        pairs=[(a,b) for a,b in tree.overlap(tree) if a<b and not set(ts[a]).intersection(ts[b]) and ((max(ts[a])<8060 and max(ts[b])<8060) or (mats[a]==4 and mats[b]==4))]
