...
OUT=Path('C:/Users/evan4/Projects/ApocalypseRV/art_source/monster_refined_v019')
s=bpy.data.scenes['MONSTER_REFINED_V019'];bpy.context.window.scene=s
r=bpy.data.objects['Refined019_Rig'];m=bpy.data.objects['Refined019_Mesh']
 ...
 for frame in range(int(a.frame_range[0]),int(a.frame_range[1])+1):
  ...
  if t.name=='grab_stand_bite' and frame in [0,14,23,39]:
   ...
   for az,el in [(0,0),(-30,0),(30,0),(0,15),(0,-15)]:
    ...
    visible=0
    for i in range(18):
     start=len(vs)-432+i*24;tip=sum((vs[j] for j in range(start+16,start+24)),Vector())/8
