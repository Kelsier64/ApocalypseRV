...
if scene is None:
    ...
    for ob in source.objects:
        if ob.name.startswith('Review012_'):
            c=ob.copy();c.data=ob.data.copy();c.name=ob.name.replace('012','015');scene.collection.objects.link(c)
            if c.type=='CAMERA': scene.camera=c
    scene.world=source.world.copy()
else:
    out_rig=bpy.data.objects['Refined015_Rig']
    out_mesh=bpy.data.objects['Refined015_Mesh']
for track in out_rig.animation_data.nla_tracks: track.mute=True
out_rig.animation_data.action=None

def aim_bone(name, direction, twist=0):
    bpy.context.view_layer.update()
    p=rig.pose.bones[name]
    origin=p.matrix.translation.copy()
    rest=p.bone.matrix_local.to_3x3()
    direction=Vector(direction).normalized()
    q=(p.bone.tail_local-p.bone.head_local).normalized().rotation_difference(direction)
    rotation=Quaternion(direction,math.radians(twist)).to_matrix() @ q.to_matrix() @ rest
    p.matrix=Matrix.Translation(origin) @ rotation.to_4x4()
    bpy.context.view_layer.update()

def gait(name,t):
    pose(1)
    phase=t*math.tau
    stride,lift,lower,lean,swing = {'idle':(0,0,.065,12,0),'walk':(.32,.08,.08,14,.12),'chase':(.48,.19,.14,20,.20),'sprint':(.64,.27,.20,24,.24)}[name]
    move('CTRL_pelvis',(0,.03,-lower+.025*math.cos(phase*2)))
    rotate('spine_01',(1,0,0),lean)
    rotate('spine_02',(1,0,0),19)
    rotate('spine_03',(1,0,0),12)
    rotate('neck_01',(1,0,0),20)
    rotate('neck_02',(1,0,0),12)
    # Lift the chin only 18 degrees from v014; keep the bowed neck unchanged.
    rotate('head',(1,0,0),-36)
    rotate('spine_01',(0,0,1),(1 if name=='idle' else 3)*math.sin(phase))
    for side,sign in [('L',1),('R',-1)]:
        wave=math.sin(phase)*sign
        move('CTRL_foot_'+side,(sign*.025,-stride*wave,lift*max(0,math.cos(phase)*sign)**2))
        # FK directions keep elbows behind the ribs instead of an IK pole
        # flaring them out; wrist twist turns palms toward the body.
        for bone in ['forearm_'+side,'hand_'+side]:
            for c in rig.pose.bones[bone].constraints: c.influence=0
        aim_bone('upper_arm_'+side,(sign*.12,.13+swing*wave,-.48))
        aim_bone('forearm_'+side,(sign*.06,-.10+swing*wave*.6,-.49))
        aim_bone('hand_'+side,(0,-.03,-1),sign*80)
    bpy.context.view_layer.update()

deform=[p.name for p in rig.pose.bones if p.bone.use_deform]
for name,seconds in [('idle',2.4),('walk',1.2),('chase',.8),('sprint',.6)]:
    for track in list(out_rig.animation_data.nla_tracks):
        if track.name==name: out_rig.animation_data.nla_tracks.remove(track)
    action=bpy.data.actions.new('raker_v015_'+name)
...
OUT='C:/Users/evan4/Projects/ApocalypseRV/art_source/monster_refined_v015/'
for name,frame in [('idle',0),('walk',9),('chase',6),('sprint',4)]:
    ...
    scene.camera.location=(6,0,1.8)
    scene.camera.rotation_euler=(Vector((0,-.3,.95))-scene.camera.location).to_track_quat('-Z','Y').to_euler()
    scene.render.filepath=OUT+name+'_side.png'
    bpy.ops.render.render(write_still=True)
print('AUTHORED v015')
