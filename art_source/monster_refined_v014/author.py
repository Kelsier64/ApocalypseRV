...
if scene is None:
    ...
    for ob in source.objects:
        if ob.name.startswith('Review012_'):
            c=ob.copy();c.data=ob.data.copy();c.name=ob.name.replace('012','014');scene.collection.objects.link(c)
            if c.type=='CAMERA': scene.camera=c
    scene.world=source.world.copy()
else:
    out_rig=bpy.data.objects['Refined014_Rig']
    out_mesh=bpy.data.objects['Refined014_Mesh']
...
for name,seconds in [('idle',2.4),('walk',1.2),('chase',.8),('sprint',.6)]:
    ...
    action=bpy.data.actions.new('raker_v014_'+name)
...
OUT='C:/Users/evan4/Projects/ApocalypseRV/art_source/monster_refined_v014/'
