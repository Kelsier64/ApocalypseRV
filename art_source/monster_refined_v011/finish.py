...
scene=bpy.data.scenes['MONSTER_REFINED_V011']
bpy.context.window.scene=scene
rig=bpy.data.objects['Refined011_Rig']
mesh=bpy.data.objects['Refined011_Mesh']
...
tex.inputs['Scale'].default_value=29
image=bpy.data.images.new('Raker011_Albedo',width=2048,height=2048,alpha=False)
...
image.filepath_raw=OUT+'raker_albedo_2k.png'
...
for i,mat in enumerate(list(mesh.data.materials)):
    ...
    baked=mat.copy()
    baked.name='Raker011_BakedSkin' if i==0 else 'Raker011_BakedEyes'
...
rig.name='Raker_Rig'
try:
    bpy.ops.export_scene.gltf(filepath=OUT+'raker_refined_v011.glb',use_selection=True,use_active_scene=True,
