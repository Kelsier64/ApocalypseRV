...
6. RV climbing flow
   ...
   - Climb start is disabled when the upward ceiling ray reports overhead obstruction (prevents entering climb in low headroom interiors).
   ...
   - Upward climb now uses a ceiling ray clamp; if overhead is detected while climbing up (W), climb exits immediately (direct detach) instead of continuing into ceiling geometry.
