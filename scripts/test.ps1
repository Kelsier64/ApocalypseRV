...
foreach ($test in $tests) {
    ...
    if ($test.BaseName -like 'test_outdoor_*' -or $test.BaseName -in @('test_rv_handling', 'test_tire_handling', 'test_checkpoint_failures', 'test_interior_traversal', 'test_raker_cabin', 'test_raker_neck', 'test_raker_grab_vehicle')) { $testArguments += @('--fixed-fps', '60') }
